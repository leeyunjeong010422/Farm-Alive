...
public class FusionSpawn : NetworkBehaviour
{
    ...
    [SerializeField] private GameObject _leftControllerObject;
    [SerializeField] private GameObject _rightControllerObject;
    ...
    private void Update()
    {
        if(!_leftControllerObject.activeSelf)
            _leftControllerObject.SetActive(true);

        if (!_rightControllerObject.activeSelf)
            _rightControllerObject.SetActive(true);
    }
}
