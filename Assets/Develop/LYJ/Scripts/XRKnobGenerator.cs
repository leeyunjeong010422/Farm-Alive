...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    /// </summary>
    public class XRKnobGenerator : XRBaseInteractable
    {
        ...
        private IEnumerator ReturnKnobToStart()
        {
            float returnSpeed = 2f; // 원위치로 돌아가는 속도
            float targetValue = 0f; // 노브가 돌아가야 할 목표 값

            while (Mathf.Abs(m_Value - targetValue) > 0.01f)
            {
                m_Value = Mathf.MoveTowards(m_Value, targetValue, Time.deltaTime * returnSpeed);
                SetKnobRotation(ValueToRotation());
                yield return null;
            }
